...
Inigo Quilez Articles and videos (utility functions)
...
float map(in vec3 pt, out Material mat, in bool includeDielectrics) {
    ...
    // OPAQUE
    {
        ...
        // ground
        float ground = pt.y - 0.006;

        // room
        float room;
        {
            float s = 1.0;
            room = sdfBox(pt - vec3(-s, 0, 0), vec3(0.1, s, s));
            room = smoothmin(room, sdfBox(pt - vec3(0, 0, -s), vec3(s, s, 0.1)), 0.01);
        } 
        ...
        {
            // coffee cup main
            vec2 p = vec2(-0.66, -0.66);
            float r = 0.35*pt.y + smoothstep(0.0, 0.008, pt.y) * 0.009;
            cup = distance(pt.xz, p) - 0.03 - r;
            float insideCup = distance(pt.xz, p) - 0.024 - r;
            cup = max(cup, -insideCup);
            cup = max(cup, -(pt.y - 0.01));
            cup = smoothmax(cup, pt.y - 0.047, 0.005);
            // right eye paint
            vec3 re = vec3(p.x + 0.04, 0.035, p.y + 0.025);
            paint = sdfSphere(pt - re, 0.0065);
            // left eye
            vec3 le = re;
            le.z -= 0.02;
            paint = min(paint, sdfSphere(pt - le, 0.0125));
            // mouth
            vec3 mouth = vec3(0.5*(re.x+le.x)-0.005, re.y-0.02, 0.5*(re.z+le.z));
            mouth.y += 0.5*distance(vec2(pt.x, pt.z), vec2(mouth.x, mouth.z));
            paint = min(paint, sdfEllipsoid(pt - mouth, 1.2*vec3(0.01, 0.002, 0.01)));
        }
        ...
        res = smoothmin(room, ground, 0.005);
        res = min(res, cup);
        
        if (res == room) {
            mat.clr = vec3(0.03, 0.02, 0.02) * 1.2;
            mat.shininess = 10.0;
            mat.ref = vec3(0.0);
            mat.id = 1.0;
        } else if (res == ground) {
            mat.clr = vec3(0.07, 0.02, 0.01) * 0.2;
            mat.shininess = 50.0;
            mat.ref = vec3(0.0);
            mat.id = 0.0;
        } else if (res == cup) {
            mat.clr = vec3(0.08, 0.08, 0.075);
            mat.shininess = 100.0;
            mat.ref = vec3(0.0);
            mat.id = 2.0;
            if (paint < cup) {
                mat.clr = vec3(0.02, 0.02, 0.01) * 1.2;
            }
        }
    ...
    if (includeDielectrics) {
        ...
        p.z += 0.66;
        p.y -= 0.04;
        float d = sdfEllipsoid(p, vec3(0.045, 0.01, 0.045));
        
        if (d < res) {
            res = d;
            mat.id = 3.0;
            mat.IOR = 1.3;
            mat.ref = vec3(1.0, 1.0, 1.0);
        ...
        p.x -= -0.6;
        p.y -= 0.06;
        float d2 = sdfSphere(p, 0.03);
        
        if (d2 < res) {
            res = d2;
            mat.IOR = 1.45;
            mat.ref = vec3(0.01, 1.0, 1.0);
            mat.isDielectric = true;
...
vec3 calcClr(in vec3 ro, in vec3 rd, in float d, in vec3 nor, in Material mat) {
    ...
    // point light
    Light li;
    li.clr = vec3(8.8, 8.8, 4.0);
    li.dir = normalize(vec3(1, 1, 1));
    li.shadowSoftness = 10.0;
    ...
    // CALCULATE COLOR
    float occ = calcOcc(pt, nor, false); //occ with dielectrics
    // float occ = calcOcc(pt, nor, false); // occ without dielectrics
    vec3 light = vec3(mat.amb);
    light += calcLighting(pt, rd, nor, li, mat, true) * occ; // main light
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // target
    vec3 target = vec3(-0.95, -0.05, -0.95);
    // ray origin
    vec3 ro = vec3(-0.53, 0.07, -0.62);
