...
Link: https://www.shadertoy.com/view/cs3fWH
...
float sdf(in vec3 pt, out Material mat) {
    ...
    // ground
    {
        res = pt.y - 0.3 * (1.2 + 1.5*sin(length(pt.xz*5.0))) * noise(pt.xz * 40.0);
        mat.id = 0;
        mat.hasFresnel = false;
    }
    ...
    {
        vec3 q = pt;
        float w = dot(q, vec3(1, 0.5, 0)) + 1.4 + 0.4 * min(abs(tan(pt.z * 5.0)), 10.);
        res = smoothmin(w, res, 0.8) * 0.8;// * 0.1;// DANGER
        mat.id = 1;
        mat.hasFresnel = false;
    }
    
    mat.clr = vec3(0.1, 0.05, 0.1) * 0.12;
    mat.clr = mix(mat.clr, 0.5*vec3(1, 1, 0.2), 0.15 * (pt.y));
...
vec3 calcClr(in vec3 ro, in vec3 rd, in float d, in vec3 nor, in Material mat) {
    ...
    // KEY LIGHT
    Light sun = createLight(pt, false, 1.0, 10.9, normalize(vec3(3, 1.0 - sin(iTime), 0)), vec3(29.8 + max(0.0,sin(iTime)*5.0), 6.0 + max(0.0,sin(iTime)*5.0), 4.7));
    Light ptLight = createLight(pt, true, 0.8, 5.9, vec3(0.1, 3, 2), vec3(5.0, 25, 10));
    Light back = createLight(pt, false, 1.0, 50.9, normalize(vec3(3, 1.9, -1)), vec3(4.7, 6.0, 17.0));
    ...
    light += calcLighting(pt, rd, nor, back, mat) * occ; // white light
    light += 10.0 * (uclamp(dot(nor, vec3(0, 1, 0))) * 0.5 + 0.5) * skyClr * occ; // sky diffuse
