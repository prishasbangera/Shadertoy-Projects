...
Raytracing but better

Prisha B.

Link: https://www.shadertoy.com/view/ctXcW2
...
#define AA 8 //sqrt of number of rays per pixel
...
Object calcPlane(in vec3 ro, in vec3 rd, in vec3 planeOrigin, in vec3 planeDir) {

    Object plane = defaultObject();
    planeDir = normalize(planeDir);
    plane.nor = planeDir;
    
    float denom = dot(planeDir, ro);
    
    if (denom > 1e-6) {
        
        vec3 ropt = planeOrigin - ro;
        float t = dot(ropt, planeDir) / denom;
        
        plane.type = 0;

        plane.t = min(MAX_DIST, t);
...
Object map(in vec3 ro, in vec3 rd) {
    ...
    // tiny sphere // red
    {
        Object s2 = calcSphere(ro, rd, vec3(0.4, -0.35, 0.3), 0.05);
    ...
    // sunny spherey
    {
        Object d = calcSphere(ro, rd, vec3(-1.3, 1.0, -10.0), 5.0);
        d.clr = vec3(0.2, 0.2, 0.9) * 1.0;
    ...
    // plane test
    {
        //Object pl = calcPlane(ro, rd, vec3(0, -2, 0), vec3(0, 1, 0));
        //pl.clr = vec3(1);
        //objUnion( obj, pl );
    }
...
vec3 render(in vec2 uv, in vec3 ro, in vec3 rd) {
    ...
    vec3 skyClr = vec3(rd.y + 1.0) * vec3(0.3, 0.4, 1.0) * 0.4;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (int j = 0; j < AA; j++) 
    {
        ...
        //vec3 ro = vec3(sin(an) * rad, 0.25, cos(an) * rad);
        vec3 ro = vec3(0.85, -0.1, 0.15);
