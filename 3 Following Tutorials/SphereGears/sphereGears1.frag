...
Link: https://www.shadertoy.com/view/cdsfWN
...
vec4 sdGear(in vec3 pt, in float time, in float offset) {
    ...
    // gear teeth
    float t = sdBox(q.xz - vec2(0.44, 0), vec2(0.09, 0.045)) - 0.02;

    // find distance to ring
    float ring = abs(length(pt.xz) - 0.4) - 0.07;
    t = min(t, ring);

    // cross
    float h = 1.3;
    float c = sdCross(pt - vec3(0.0, h, 0.0), vec3(0.35, 0.015, 0.015)) - 0.01;
    t = min(t, c);
...
vec3 render(in vec2 uv, in vec3 ro, in vec3 rd) {
    ...
    if (dist > 0.0) {
        ...
        vec3 mat = 0.5 * texture(iChannel0, intersection.yw).rgb + 0.5 * texture(iChannel0, intersection.yz).rgb;
        mat = 0.8 * mat + 0.2;
        //if (length(pt) < 0.36) mat = vec3(0.3, 0.2, 0.1) * 1.8;
        mat = mat*mat;
        
        vec3 light = vec3(0.0);
        light += 2.0 * vec3(1.0, 1.0, 1.1) * (0.5 + 0.5*nor.y);
        //light += 0.3 * vec3(6, 5.5, 2) * clamp(dot(nor, vec3(8, 8, 2)), 0.0, 1.0);
        
        clr = mat * light * calcAO(pt, nor);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (int j = 0; j < AA; j++) {
        ...
        vec3 ro = vec3(sin(an) * rad, 0.5, cos(an) * rad);
        vec3 target = vec3(0);
